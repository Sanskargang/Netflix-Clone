...
export function TvSeries({ props, item }) {
    ...
    function TredningMovies() {
        (async () => {
            try {
                await fetch("https://api.themoviedb.org/3/tv/top_rated?api_key=6259379dd6aa822c21edcfce778e034a")
                    .then(res => res.json())
                    ...
                    {MovieComming.slice(1, 2).map((data) => {
                        return (
                            <FlashList data={MovieComming}
                                ...
                                initialNumToRender={5} // this is used for memory saving
...
export const DataSaving = memo(TvSeries);