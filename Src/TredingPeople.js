...
export function TredningPeople({ props, item }) {
    const navigation = useNavigation()
    const [MovieComming, setMovieComming] = useState([]);
    const [isLoading, setLoading] = useState(true);
    const ShimmerPlaceholder = createShimmerPlaceholder(LinearGradient)
    function TredningMovies() {
        (async () => {
            try {
                await fetch("https://api.themoviedb.org/3/tv/airing_today?api_key=6259379dd6aa822c21edcfce778e034a")
                    .then(res => res.json())
                    ...
                    {MovieComming.slice(1, 2).map((data) => {
                        return (
                            <FlashList data={MovieComming}
                                ...
                                initialNumToRender={10} // this is used for memory saving
...
export const DataSaving = memo(TredningPeople);